...
async def pmguard(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "off":
        await Shashank.set_pm(False)
        await message.edit("**PM Guard Deactivated**")
    if arg == "on":
        await Shashank.set_pm(True)
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await Shashank.set_permit_message(Shashank.PMPERMIT_MESSAGE)
        await message.edit("**Anti_PM message set to default**.")
        return
    await Shashank.set_permit_message(f"`{arg}`")
